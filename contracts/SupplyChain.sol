...
/* Create a modifer that checks if the msg.sender is the owner of the contract */
  ...
  modifier forSale(uint _sku) {
    Item storage item_to_sell = items[_sku];
    require (item_to_sell.state == State.ForSale,'not for sale');
    require(_sku < skuCount);
    _;
  }
  modifier sold(uint _sku) {
    Item storage item_to_sell = items[_sku];
    require (item_to_sell.state == State.Sold,'not sold');
    _;
  }
  modifier shipped(uint _sku) {
    Item storage item_to_sell = items[_sku];
    require (item_to_sell.state == State.Shipped,'not shipped');
    _;
  }
  modifier received(uint _sku) {
    Item storage item_to_sell = items[_sku];
    require (item_to_sell.state == State.Received,'not received');
    _;
  }
  ...
  function buyItem(uint sku)
    public
    payable forSale(sku) paidEnough(msg.value) checkValue(sku)  
  {
